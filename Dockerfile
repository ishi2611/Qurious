...
ENV PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1 PIP_NO_CACHE_DIR=1 \
    HF_HOME=/app/.cache/huggingface FASTEMBED_CACHE_PATH=/app/.cache/fastembed \
    PORT=7860 QURIOUS_CONTENT_DIR=/app/content
...
# Bake the ONNX embedding model and the lesson embeddings into the image, so the tutor neither
# downloads the model nor re-embeds every lesson at startup (or when a free server wakes up).
RUN cd api && python -c "from app.tutor.retrieval import build_index; build_index()"
